...
class ActionFetchYoutubeVideos(Action):
    ...
    def __init__(self):
        ...
        self.youtube = build('youtube', 'v3', developerKey='AIzaSyCs5fCCifDsiaY19R0j-YaYgFR_5QEYFBg')
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        try:
            ...
            videos.sort(key=lambda x: (x['edu_score'], x.get('engagement', 0)), reverse=True)
            ...
            # Create response message for top videos
            response = f"Here are 2 educational videos about {topic}:\n\n"

            for video in top_videos:
                # Make the URL clickable by using Markdown syntax for a clickable link
                response += f"{video['title']}\n"
                response += f"Channel: {video['channel']}\n"
                response += f"[Click to watch the video]({video['url']})\n\n"  # Proper Markdown syntax for clickable link

            # Send the message with clickable links
